...
public class ContainerFactoryController extends Container
{
    ...
    public ContainerFactoryController(InventoryPlayer inventoryPlayer, IInventory inventory)
    {
        ...
        this.addSlotToContainer(new RobotModuleSlot(inventory, 0, 26, 18));
        this.addSlotToContainer(new RobotChipsetSlot(inventory, 1, 26, 36));
        this.addSlotToContainer(new FilteredIconSlot(inventory, 2, 26, 54, ResourceHelper.getModResource("textures/gui/slotArmor.png"), new StackFilter(Item.plateChain.itemID), new StackFilter(Item.plateDiamond.itemID), new StackFilter(Item.plateGold.itemID), new StackFilter(Item.plateIron.itemID)));
        this.addSlotToContainer(new OutputSlot(inventory, 3, 171, 96));
        this.addSlotToContainer(new SlotEnergy(inventory, 4, 5, 61));

        for (int inventoryRowIndex = 0; inventoryRowIndex < PLAYER_INVENTORY_ROWS; ++inventoryRowIndex)
        {
            for (int inventoryColumnIndex = 0; inventoryColumnIndex < PLAYER_INVENTORY_COLUMNS; ++inventoryColumnIndex)
            {
                this.addSlotToContainer(new Slot(inventoryPlayer, inventoryColumnIndex + inventoryRowIndex * 9 + 9, 27 + inventoryColumnIndex * 18, 118 + inventoryRowIndex * 18));
            }
        }
        ...
        {
            this.addSlotToContainer(new Slot(inventoryPlayer, actionBarSlotIndex, 27 + actionBarSlotIndex * 18, 176));
        }
    }
